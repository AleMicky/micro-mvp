...
async def reporte_dia(fecha: str | None = Query(None), db: AsyncSession = Depends(get_db)):
    return await venta_service.reporte_dia(db, fecha)
...
async def crear(payload: VentaCreate, db: AsyncSession = Depends(get_db)):
    return await venta_service.procesar_venta_completa(db, payload)
