...
def generar_pdf_pedido(venta: dict, carrito: dict, numero_whatsapp: str) -> bytes:
    pdf = FPDF()
    pdf.add_page()
    pdf.set_font("Helvetica", "B", 16)
    pdf.cell(0, 10, "Pedido WhatsApp", ln=True)

    pdf.set_font("Helvetica", "", 11)
    pdf.cell(0, 8, _ascii_seguro(f"Codigo de venta: {venta['codigo']}"), ln=True)
    ...
    pdf.cell(0, 10, f"TOTAL: Bs. {float(venta['total']):.2f}", align="R")
