...
import type { Caja } from '@/types/finanzas.types'
...
const deleteTarget = ref<Caja | null>(null)
...
async function loadData() {
  loading.value = true
  try {
    const { data } = await finanzasService.getCajas()
    items.value = data
  } catch (error) {
...
async function saveItem() {
  ...
  try {
    ...
    if (editingId.value) {
      await finanzasService.updateCaja(editingId.value, payload)
      appStore.showSuccess('Registro actualizado')
    } else {
      await finanzasService.createCaja(payload)
...
async function confirmDelete() {
  ...
  deleting.value = true
  try {
    await finanzasService.deleteCaja(deleteTarget.value.id)
