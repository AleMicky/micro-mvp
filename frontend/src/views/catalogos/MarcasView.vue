...
import type { Marca } from '@/types/catalogos.types'
...
async function loadData() {
  loading.value = true
  try {
    const { data } = await catalogosService.getMarcas()
    items.value = data
  } catch (error) {
...
function openDelete(item: Marca) {
...
async function saveItem() {
  ...
  try {
    ...
    if (editingId.value) {
      await catalogosService.updateMarca(editingId.value, payload)
      appStore.showSuccess('Marca actualizada')
    } else {
      await catalogosService.createMarca(payload)
      appStore.showSuccess('Marca creada')
    }
    dialog.value = false
    await loadData()
  } catch (error) {
...
async function confirmDelete() {
  ...
  deleting.value = true
  try {
    await catalogosService.deleteMarca(deleteTarget.value.id)
    appStore.showSuccess('Marca eliminada')
...
<template>
  ...
  <v-dialog v-model="dialog" max-width="560" persistent>
    <v-card>
      <v-card-title>{{ editingId ? 'Editar marca' : 'Nueva marca' }}</v-card-title>
      <v-card-text>
        <v-row>
          <v-col cols="12" md="6">
